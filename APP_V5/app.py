...
DB_PATH = os.path.join(BASE_DIR, "db.pkl")
SP_PATH = os.path.join(BASE_DIR, "sp_index.pkl")
...
with tab_id:
    ...
    if up is not None:
        ...
        if not sp_index:
            st.warning(
                "`sp_index.pkl` not found. Run `python build_db.py` locally, "
                "then commit `sp_index.pkl` to your repo."
            )
        else:
            st.markdown(
                "Single-peak matching counts how many query frequency bins appear "
                "in each song (no time pairing), so common bins inflate scores for "
                "every song and the winner is unreliable. Pair-hash matching anchors "
                "two peaks in time, making coincidental collisions exponentially rarer."
            )
